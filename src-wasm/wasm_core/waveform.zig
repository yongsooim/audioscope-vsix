...
fn computeStableLevelSlicePlan(level: *const core.WaveLevel, start_sample: f64, end_sample: f64, _: i32) StableLevelSlicePlan {
    ...
    var actual_end_block = core.minI32(
        level.block_count,
        core.maxI32(actual_start_block + 1, @as(i32, @intFromFloat(@ceil(desired_end_block)))),
    );

    if (actual_end_block <= actual_start_block) {
        actual_end_block = core.minI32(level.block_count, actual_start_block + 1);
    }

    if (actual_end_block > level.block_count) {
        const block_span = actual_end_block - actual_start_block;
        actual_end_block = level.block_count;
        actual_start_block = core.maxI32(0, actual_end_block - block_span);
    }
...
fn computeStableSampleSlicePlan(start_sample: f64, end_sample: f64, _: i32) StableSampleSlicePlan {
    ...
    var actual_end_sample_index = core.minI32(
        core.g_session.sample_count,
        core.maxI32(actual_start_sample_index + 1, @as(i32, @intFromFloat(@ceil(end_sample)))),
    );

    if (actual_end_sample_index <= actual_start_sample_index) {
        actual_end_sample_index = core.minI32(core.g_session.sample_count, actual_start_sample_index + 1);
    }

    if (actual_end_sample_index > core.g_session.sample_count) {
        const sample_span = actual_end_sample_index - actual_start_sample_index;
        actual_end_sample_index = core.g_session.sample_count;
        actual_start_sample_index = core.maxI32(0, actual_end_sample_index - sample_span);
    }
