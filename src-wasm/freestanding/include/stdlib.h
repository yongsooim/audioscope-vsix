...
typedef int (*audioscope_compare_fn)(const void *left, const void *right);

void *malloc(size_t size);
void *calloc(size_t count, size_t size);
void free(void *ptr);

static inline void audioscope_swap_bytes(unsigned char *left, unsigned char *right, size_t size) {
...
static inline void qsort(void *base, size_t count, size_t size, audioscope_compare_fn compare) {
  ...
  for (size_t gap = count / 2; gap > 0; gap /= 2) {
    for (size_t index = gap; index < count; index += 1) {
      ...
      while (inner >= gap) {
        ...
        audioscope_swap_bytes(left, right, size);
        inner -= gap;
      }
    }
  }
